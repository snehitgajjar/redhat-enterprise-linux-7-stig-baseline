control "V-204575" do
  title 'The Red Hat Enterprise Linux operating system must be configured so that the rsyslog daemon does not accept log
    messages from other servers unless the server is being used for log aggregation.'
  desc "Unintentionally running a rsyslog server accepting remote messages puts the system at increased risk. Malicious
    rsyslog messages sent to the server could exploit vulnerabilities in the server software itself, could introduce
    misleading information in to the system's logs, or could fill the system's storage leading to a Denial of Service.
...
document the system as being used for log aggregation."
  ...
  tag 'gid': 'V-204575'
  tag 'rid': 'SV-204575r505924_rule'
  tag 'stig_id': 'RHEL-07-031010'
  tag 'fix_id': 'F-4699r88918_fix'
  tag 'cci': %w(CCI-000318 CCI-001812 CCI-001813 CCI-001814 CCI-000368)
